...
using WoWmapperX.Controllers.Xbox;
...
namespace WoWmapperX.Controllers
{
    public static partial class ControllerManager
    {
        ...
        public static Bitmap GetButtonIcon(GamepadButton button)
        {
            switch (AppSettings.Default.ButtonStyle)
            {
                case 0:
            ...
            foreach (var controller in controllerList)
            {
                lock (Controllers)
                {
                    ...
                    if (ControllersChanged != null)
                        Avalonia.Threading.Dispatcher.UIThread.Post(() => ControllersChanged());
                }
            }
        }
        ...
        public static void SetController(IController controller)
        {
            ...
            if (ActiveControllerChanged != null)
                Avalonia.Threading.Dispatcher.UIThread.Post(() => { ActiveControllerChanged?.Invoke(controller); });

            WoWmapperX.ConsolePort.BindWriter.WriteBinds();
        ...
        {
            if (!AppSettings.Default.EnableTouchpad) return;

            // Ignore touchpad if disabled during mouselook
            // Ignore touchpad if disabled during mouselook
            if (AppSettings.Default.MemoryTouchpadCursorOnly &&
                WoWReader.IsAttached &&
                WoWReader.GameState &&
                WoWReader.MouselookState)
                return;

            switch (AppSettings.Default.TouchpadMode)
            {
                case 0:
        ...
        private static void DS4TouchpadMoved(object sender, TouchpadEventArgs args)
        {
            ...
            var cur = new Point(Cursor.Position.X + x, Cursor.Position.Y + y);
            Cursor.Position = cur;
        }
        ...
        private static void ControllerWatcher()
        {
            while (_threadRunning)
            {
                lock (Controllers)
                {
                    ...
                    {
                        SetController(null);
                        Avalonia.Threading.Dispatcher.UIThread?.Post(() => { ActiveControllerChanged?.Invoke(null); });
                    }
                    ...
                    if (AppSettings.Default.EnableOverlay && AppSettings.Default.EnableOverlayBattery && ActiveController != null)
                    {
